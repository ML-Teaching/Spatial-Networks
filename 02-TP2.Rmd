...
Avant de poursuivre les analyses, prenons un instant pour aborder la notion de 
**réseau**. Il existe deux principales façons de représenter un réseau dirigé et
pondéré comme celui que nous étudions ici. 

- **Le format matriciel** : une matrice carrée où chaque case représente la 
  valeur du flux entre une origine (ligne) et une destination (colonne). C'est 
  le format que nous utilisons avec **od_mtp**.  
...
d'autres outils spécialisé dans l'analyse de graphe (comme **igraph** par 
exemple).

> *Particularité importante : les liens inexistants 
...
mat_mtp2 <- net_to_mat(net_mtp, weight = TRUE, square = TRUE)

mat_mtp2[1:10,1:10]
mat_mtp[1:10,1:10]

# Nombre total de paires et nombre de paires "nulles"
length(mat_mtp)
sum(mat_mtp==0)
```
